...
export default class FeatureFlagController {
  ...
  createFlag = asyncHandler(async (req: OrgContextRequest, res: Response): Promise<void> => {
    ...
    const result = await this.featureFlagService.createFlag(dto, req.user.orgId);
  ...
  listFlags = asyncHandler(async (req: OrgContextRequest, res: Response): Promise<void> => {
    ...
    const result = await this.featureFlagService.listFlags(environment.LOCAL, req.user.orgId);
  ...
  getFlagByEnvironment = asyncHandler(async (req: OrgContextRequest, res: Response): Promise<void> => {
    ...
    const result = await this.featureFlagService.getFlag(keyStr, envStr as typeof environment[keyof typeof environment], req.user.orgId);
  ...
  updateFlag = asyncHandler(async (req: OrgContextRequest, res: Response): Promise<void> => {
    ...
    const result = await this.featureFlagService.updateFlag(key, dto, req.user.orgId);
  ...
  deleteFlag = asyncHandler(async (req: OrgContextRequest, res: Response): Promise<void> => {
    const { key } = req.params as { key: string };
    ...
    await this.featureFlagService.deleteFlag(key, req.user.orgId);
    logger.info('Feature flag deleted successfully', { key });
    res.status(204).send();
  });

  enableFlagForEnvironment = asyncHandler(async (req: OrgContextRequest, res: Response) => {
    const { key } = req.params;
    const keyStr: string = Array.isArray(key) ? (key[0] as string) : (key as string);
    ...
    const result = await this.featureFlagService.enableFlag(keyStr, envStr as typeof environment[keyof typeof environment], req.user.orgId);
  ...
  disableFlagForEnvironment = asyncHandler(async (req: OrgContextRequest, res: Response) => {
    const { key } = req.params;
    const keyStr: string = Array.isArray(key) ? (key[0] as string) : (key as string);
    ...
    const result = await this.featureFlagService.disableFlag(keyStr, envStr as typeof environment[keyof typeof environment], req.user.orgId);
